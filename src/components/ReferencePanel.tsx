...
export const ReferencePanel: React.FC<ReferencePanelProps> = ({ isOpen, onClose }) => {
    const [activeTab, setActiveTab] = useState<Tab>('scales');

    if (!isOpen) return null;

    return (
        <div className="fixed inset-0 z-50 bg-gray-50 dark:bg-gray-900 overflow-auto animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col items-center">
            <div className="w-full max-w-6xl p-4 md:p-8">
                <div className="flex items-center gap-4 mb-6">
                    <button onClick={onClose} className="p-2 bg-white dark:bg-gray-800 rounded-full shadow hover:bg-gray-100 dark:hover:bg-gray-700 transition font-bold px-4">← Back</button>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Music Reference</h1>
                </div>

                <div className="w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden min-h-[600px] flex flex-col">
                    {/* Tab Navigation */}
